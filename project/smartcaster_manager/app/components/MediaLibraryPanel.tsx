          ...
          <div className="flex flex-col divide-y">
            {filtered.map((item) => (
              <div
                ...
                className={`flex items-center gap-3 px-2 py-2 cursor-move transition-all ${
          ...
          <div className={`grid ${gridCols} gap-3`}>
            {filtered.map((item) => (
              <div
                ...
                onDragStart={() => onSelect(item.name)}
                onClick={() => onSelect(item.name)}
                className={`flex flex-col items-center ${cardPadding} rounded-lg cursor-move transition-all ${
                  selectedMediaName === item.name
                    ? "bg-blue-100 ring-2 ring-blue-500"
                    : "bg-white hover:bg-gray-100"
                }`}
              >
                <div className={`${iconSize} mb-2`}>{getFileIcon(item.type)}</div>
